...
def main_kodish():
    ...
    #super_log_fix()
    dialog = xbmcgui.Dialog()
    link = dialog.select('Kodish Store - Neomix', ['Doação','Loja da Kodish','','Instalar Addons','Remover o Elementun','Instalar o Fix (Obrigatorio)','Repositorios','Instalar o Elementum','Instalar Torrest','Limpar o Super Cache','Instalar F4M Tester'])
    links = str(link)
    file = open(filewk,"a") 
    file.write(links)
    file.close()
